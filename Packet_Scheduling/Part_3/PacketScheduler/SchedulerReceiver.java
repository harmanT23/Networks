...
/**
 * Listens on specified port for incoming packets. Packets are stored to queues.
 */
public class SchedulerReceiver implements Runnable {
	...
	/**
	 * Listen on port and send out or store incoming packets to buffers. This method
	 * is invoked when starting a thread for this class.
	 */
	public void run() {
		...
		try {
			...
			while (true) {
				...
				/*
				 * Record arrival to file in following format: elapsed time (microseconds),
				 * packet size (bytes), backlog in buffers ordered by index in array (bytes).
				 */
				...
				/*
				 * TODO: Replace previous command with code that: 
				 * - implements packet classifier
				 * - stores packets to appropriate queue - reports and/or logs packets drops
				 * with all information you need
				 */

				// Traffic Classification
				if (buf[0] == 0x01) {
					pOut.print("1" + "\t"); //Denote class 1
					...
					if (buffers[0].addPacket(new DatagramPacket(packet.getData(), packet.getLength())) < 0) {
						System.err.println("Packet dropped from flow 1 (queue full).");
					}
				} else if (buf[0] == 0x02) {
					pOut.print("2" + "\t"); //Denote class 2
					if (buffers[1].addPacket(new DatagramPacket(packet.getData(), packet.getLength())) < 0) {
						System.err.println("Packet dropped from flow 2 (queue full).");
					}
				} else if (buf[0] == 0x03) {
					pOut.print("3" + "\t"); //Denote class 3
					if (buffers[2].addPacket(new DatagramPacket(packet.getData(), packet.getLength())) < 0) {
						System.err.println("Packet dropped from flow 3 (queue full).");
					}
				} else {
					System.err.println("Packet arrival from unknown stream.");
				}
				pOut.println();
			}
		} catch (Exception e) {
